"""
Point cloud classifier for main task submission - Random Forest with handcrafted geometric features.

Offline evaluation with extended feature set. No real-time constraints.
Default: 23 features, 100 trees.

Feature modes:
  - 19 features: compact (pipeline fast path)
  - 23 features: extended - main submission (includes KDTree, ConvexHull features)
  - 35 features: full set including redundant derived ratios

Usage:
  python classifier_main_task.py --features 23
  python classifier_main_task.py --features 35 --n-estimators 100 --force-extract
"""
...
# Default — updated by set_feature_mode()
FEATURE_MODE = 23
FEATURE_NAMES = FEATURES_EXTENDED

def set_feature_mode(mode: int) -> None:
    """Set global feature mode (19, 23, or 35). Updates FEATURE_NAMES accordingly."""
...
def run_cross_validation(
    ...
    """Stratified k-fold CV — development metrics."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--features", type=int, choices=[19, 23, 35], default=23,
    ...
    cache_dir = "cache"
    train_cache = os.path.join(cache_dir, f"main_train_{FEATURE_MODE}feat.npz")
    test_cache = os.path.join(cache_dir, f"main_test_{FEATURE_MODE}feat.npz")

    # --- Augment training data ---
    #logger.info("Augmenting training data with partial views...")
    #train_points, train_labels = augment_dataset(train_points, train_labels)
    ...
    model_path = f"models/main_rf_classifier_{FEATURE_MODE}feat.pkl"
